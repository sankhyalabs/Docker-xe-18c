  ...
  if [ -d "$SCRIPTS_ROOT" ] && [ -n "$(ls -A $SCRIPTS_ROOT)" ]; then
    ...
    for f in $SCRIPTS_ROOT/*; do
        case "$f" in
            ...
            *.sql)    echo "$0: running $f"; echo "exit" | su - oracle -c "$ORACLE_HOME/bin/sqlplus / as sysdba @$f"; echo ;;
            *)        echo "$0: ignoring $f" ;;
        esac
        echo "";
    done
...
function moveFiles {
   if [ ! -d $ORACLE_BASE/oradata/dbconfig/$ORACLE_SID ]; then
      su - oracle -c "mkdir - $ORACLE_BASE/oradata/dbconfig/$ORACLE_SID/"
   fi;
   
   su - oracle -c "mv $ORACLE_HOME/dbs/spfile$ORACLE_SID.ora $ORACLE_BASE/oradata/dbconfig/$ORACLE_SID/"
   su - oracle -c "mv $ORACLE_HOME/dbs/orapw$ORACLE_SID $ORACLE_BASE/oradata/dbconfig/$ORACLE_SID/"
   su - oracle -c "mv $ORACLE_HOME/network/admin/listener.ora $ORACLE_BASE/oradata/dbconfig/$ORACLE_SID/"
   su - oracle -c "mv $ORACLE_HOME/network/admin/tnsnames.ora $ORACLE_BASE/oradata/dbconfig/$ORACLE_SID/"
...
if [ -d $ORACLE_BASE/oradata/$ORACLE_SID ]; then
   ...
   if [ ! -d $ORACLE_BASE/admin/$ORACLE_SID/adump ]; then
      su - oracle -c "mkdir - $ORACLE_BASE/admin/$ORACLE_SID/adump"
   fi;
fi;
